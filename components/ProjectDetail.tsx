                ...
                <svg className="hidden lg:block absolute inset-0 w-full h-full pointer-events-none" style={{ overflow: 'visible' }}>
                  {leftAnnotations.map((a, i) => {
                    ...
                    return (
                      <g key={`l${i}`}>
                        {/* Dot on image edge */}
                        <circle cx="0" cy={`${cy}%`} r="3" fill={a.color || '#7B61FF'} opacity="0.6" />
                        {/* Line from dot to left */}
                        <line x1="0" y1={`${cy}%`} x2="-12" y2={`${cy}%`} stroke={a.color || '#7B61FF'} strokeWidth="0.75" opacity="0.35" />
                      </g>
                    );
                  })}
                  {rightAnnotations.map((a, i) => {
                    ...
                    return (
                      <g key={`r${i}`}>
                        <circle cx="100%" cy={`${cy}%`} r="3" fill={a.color || '#7B61FF'} opacity="0.6" />
                        <line x1="100%" y1={`${cy}%`} x2="100%" y2={`${cy}%`} stroke={a.color || '#7B61FF'} strokeWidth="0.75" opacity="0.35" style={{ transform: 'translateX(12px)' }} />
                      </g>
                    );
                  })}
                </svg>
