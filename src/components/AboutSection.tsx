...
const AboutSection: React.FC<AboutSectionProps> = ({ onOpenWalkThrough }) => {
  ...
  return (
    <section ref={containerRef} id="introduction" className="relative py-20 sm:py-28 md:py-32 px-6 md:px-12 bg-cream-light overflow-hidden">
      ...
      {/* Walk Through entry - bottom right arrow */}
      {onOpenWalkThrough && (
        <motion.button
          ...
          className="absolute bottom-8 right-8 sm:bottom-12 sm:right-12 z-20 group flex items-center gap-2.5 px-5 py-3 rounded-full bg-white/80 backdrop-blur-sm border border-cream-dark/50 hover:border-brown/40 hover:bg-white shadow-sm hover:shadow-md transition-all"
        >
          <span className="text-xs text-warm-gray group-hover:text-dark-brown transition-colors tracking-wide">
            {language === 'zh' ? '进入框架' : 'Enter My Frame'}
          </span>
          <ArrowRight size={14} className="text-warm-gray group-hover:text-dark-brown transition-colors" />
        </motion.button>
      )}
    </section>
  );
